...
class ScheduledMeeting(db.Model):
    ...
    def __init__(self, event_id, subject, start_date, start_time,end_date,end_time, attendees, cc_recipients, recruiter_id, recruiter_email, time_zone, join_url,description=None):
...
#     # Fetch necessary columns from the candidate_resume table
#     candidates = candidate_resume.query.with_entities(
#         candidate_resume.id,
#         candidate_resume.name,
#         candidate_resume.phone,
#         candidate_resume.email,
#         candidate_resume.skills
#     ).all()

#     def process_candidate(candidate):
#         # Unpack the tuple
#         candidate_id, name, phone, email, skills = candidate

#         if skills:
#             candidate_skills_set = set(skill.lower() for skill in skills)  # Skills is already a list
#             gemini_skills_with_role = gemini_skills.union({job_role.lower()})
#             matched_skills = list(gemini_skills_with_role.intersection(candidate_skills_set))
#             if matched_skills:
#                 match_percentage = (len(matched_skills) / len(gemini_skills_with_role)) * 100
...
#                     "email": email,
#                     "phone": phone,
#                     "candidate_skills": skills,  # Original candidate skills
#                     "matched_skills": matched_skills,  # Skills that matched with Gemini-generated skills
#                     "gemini_generated_skills": list(gemini_skills_with_role)  # Skills generated by Gemini for the job role plus job role itself
#                 }
#         return None
...
def search_resumes():
    ...
    # Fetch necessary columns from the candidate_resume table, including resume_base64
    candidates = candidate_resume.query.with_entities(
        candidate_resume.id,
        candidate_resume.name,
        candidate_resume.phone,
        candidate_resume.email,
        candidate_resume.skills,
        candidate_resume.resume_base64  # Include the resume_base64 field
    ).all()

    def process_candidate(candidate):
        # Unpack the tuple
        candidate_id, name, phone, email, skills, resume_base64 = candidate
        ...
        if skills:
            ...
            if matched_skills:
                ...
                return {
                    ...
                    "matched_skills": matched_skills,  # Skills that matched with Gemini-generated skills
                    "gemini_generated_skills": list(gemini_skills_with_role),  # Skills generated by Gemini for the job role plus job role itself
                    "resume_base64": resume_base64  # Include the resume_base64 field
                }
        return None
...
def create_event(subject, start_date, start_time, end_date, end_time, attendees, cc_recipients, recruiter_email, time_zone, description):
    access_token = get_access_token()
    ...
    event = {
        ...
        'isOnlineMeeting': True,
        'onlineMeetingProvider': 'teamsForBusiness',
        'body': {  # Add the description
            'contentType': "Text",  # You can also use 'Text' if HTML formatting isn't required
            'content': description
        }
    }
...
def handle_create_event():
    ...
    recruiter_email = data.get('recruiter_email')
    time_zone = data.get('time_zone')  # Default to UTC if not provided
    description = data.get('description', '')  # Defaults to an empty string if not provided
    ...
    event_response, error = create_event(
        ...
        recruiter_email=recruiter_email,
        time_zone="Asia/Kolkata",  # Use IST timezone
        description=description  # Pass description to the function
    )
    ...
    if event_response:
        ...
        new_meeting = ScheduledMeeting(
            ...
            time_zone="Asia/Kolkata",  # Use IST timezone
            join_url=join_url,  # Save joinUrl in the database
            description= description # Save description in the database
        )
...
def generate_questions_jd():
    ...
    add question mark at the end of each question
    present the output in below format only dont not include any explanations
    {{
    "skill1":["question1 on skill1","question2 on skill1","question3 on skill1","question4 on skill1","question5 on skill1"],
    "skill2":["question1 on skill2","question2 on skill2","question3 on skill2","question4 on skill2","question5 on skill2"],
    .
...
def verify_user():
    data = request.json
    name = data.get('name')  # Check for 'name' instead of 'username'
    ...
    if not user:
        return jsonify({'status': 'error', 'message': 'Invalid name or user does not exist.'}), 404

    if user.is_verified:
        # Check the user type and prepare the appropriate message
        if user.user_type == 'management':
            message = 'Your Management Account is already Verified. Please Login!'
            redirect_url = 'https://ats-makonis.netlify.app/ManagementLogin'
        elif user.user_type == 'recruiter':
            message = 'Your Recruiter Account is already Verified. Please Login!'
            redirect_url = 'https://ats-makonis.netlify.app/RecruitmentLogin'
    ...
    # Prepare success messages based on user type
    if user.user_type == 'management':
        message = 'Your Management Account has been Successfully Verified. Please Login!'
        redirect_url = 'https://ats-makonis.netlify.app/ManagementLogin'
    elif user.user_type == 'recruiter':
...
# Configure Flask-Mail for Microsoft Office 365 (Outlook)
# app.config['MAIL_SERVER'] = 'smtp.office365.com'
# app.config['MAIL_PORT'] = 587
# app.config['MAIL_USE_TLS'] = True
# app.config['MAIL_USE_SSL'] = False
# app.config['MAIL_USERNAME'] = 'teja.g@makonissoft.com'  # Your Office 365 email
# app.config['MAIL_PASSWORD'] = 'Mani@9908$'  # Your Office 365 email password (consider using environment variables in production)
# app.config['MAIL_DEFAULT_SENDER'] = 'teja.g@makonissoft.com'  # Default sender address
# app.config['MAIL_REPLY_TO'] = 'teja.g@makonissoft.com'  # Reply-to address

# # Initialize Flask-Mail with the app
# mail = Mail(app)

# # Enable debug mode for Flask-Mail
...
#     try:
#         mail.send(msg)
#         app.logger.info(f"Verification email sent to {new_user.email}")
#     except Exception as e:
#         app.logger.error(f"Error sending email to {new_user.email}: {str(e)}")
#         return f'Failed to send mail: {str(e)}'
#     return None
...
def dashboard():
    ...
    page_no = 1
    all_candidates = []  # Initialize an empty list to store all candidates
    has_more = True  # Variable to check if there are more records to fetch

    response_data = {}

    # Start iterating and fetching candidates in batches
    while has_more:
        ...
        if user_type == 'recruiter':
            ...
            # Add the current batch of candidates to the all_candidates list
            all_candidates.extend(candidates_batch)

            # Check if there are more candidates to fetch
            total_candidates = db.session.query(Candidate).filter(and_(Candidate.recruiter == recruiter.name, Candidate.reference.is_(None))).count()
            has_more = (page_no * BATCH_SIZE) < total_candidates
        ...
        else:  # For other user types (like a specific recruiter)
            candidates_query = Candidate.query.filter_by(recruiter=user.name)\
                .order_by(
                    desc(conditional_order_date),
                    desc(conditional_order_time),
    ...
    response_data = {
        ...
        'user_name': user_name,
        'name': name,
        'candidates': [get_common_candidate_data(candidate) for candidate in all_candidates],  # Return all candidates
        'jobs': [get_common_job_data(job) for job in jobs],
        'page_no': page_no,
        'has_more': has_more,  # Indicating if there are more candidates to fetch
        'edit_candidate_message': data.get('edit_candidate_message'),
...
def view_all_jobs():
    ...
    # Retrieve job posts with conditional ordering
    job_posts_active = JobPost.query.filter_by(job_status='Active')\
        .order_by(
            desc(conditional_order_date),
            desc(conditional_order_time),
    ...
    job_posts_closed = JobPost.query.filter_by(job_status='Close')\
        .order_by(
            desc(conditional_order_date),
            desc(conditional_order_time),
            desc(JobPost.id)
    ...
    response_data = {
        ...
        "job_posts_closed": [
            ...
            for job_post in job_posts_closed
        ]
    }
...
def store_resume_data(resume_dict, resumes):
    ...
    for key, value in resume_dict.items():
        ...
        if len(value) >= 4:  # Ensure we have at least 4 skills
            ...
            # Format the cleaned skills as a PostgreSQL array literal
            formatted_skills = '{' + ','.join(cleaned_skills) + '}'
            ...
            new_candidate = candidate_resume(
                ...
                phone=phone,
                skills=cleaned_skills,  # Ensure this is a list for the skills
                resume_base64=resume64  # Store the base64 resume
            )
...
#     for batch_start in range(0, len(resumes), 5):
#         batch_resumes = resumes[batch_start:batch_start + 5]
#         batch_index = 1  # Reset batch index
...
#                 all_resumes_text += formatted_text
#                 extracted_results.append({'index': batch_start + batch_index - 1, 'extracted_text': formatted_text})
#                 batch_index += 1
#             except Exception as e:
#                 extracted_results.append({'index': batch_start + batch_index - 1, 'error': str(e)})
...
def get_all_candidates():
    try:
        # Fetch all records from the candidate_resume table
        all_candidates = candidate_resume.query.all()

        # Convert database records into dictionaries for JSON response
        candidates_data = [
            {
                'id': candidate.id,
                'name': candidate.name,
                ...
                'skills': candidate.skills,
                'resume_base64': candidate.resume_base64  # Include the resume data
            }
        ...
        return jsonify({
            'status': 'success',
            'candidate_records': candidates_data  # Send all candidate records in the response
        }), 200
    except Exception as e:
        return jsonify({
            'status': 'error',
...
def view_resume_research(candidate_id):
    ...
    candidate = candidate_resume.query.filter_by(id=candidate_id).first()
    
    # Check if candidate exists
    if not candidate:
        return 'Candidate not found', 404

    # Check if the resume_base64 field is empty or None
    if candidate.resume_base64 is None or candidate.resume_base64.strip() == '':
        return 'Resume not found for this candidate', 404

    try:
        # Decode the base64 encoded resume
        resume_binary = base64.b64decode(candidate.resume_base64)

        # Determine if the resume is a PDF (by checking the %PDF signature)
        is_pdf = resume_binary.startswith(b"%PDF")
        mimetype = 'application/pdf' if is_pdf else 'application/msword'

        # Send the file as a response to be viewed in the browser
        return send_file(
            io.BytesIO(resume_binary),
            mimetype=mimetype,
            as_attachment=False
...
def get_all_candidates_dashboard():
    ...
    response_data = [convert_candidate_to_dict(candidate) for candidate in all_candidates]
    ...
    for candidate in selected_candidates:
        ...
        if candidate_record and candidate_record.resume:
            try:
                ...
                return jsonify({
                    "status": "failure",
                    "message": f"Error processing resume for candidate ID {candidate_id}: {str(e)}"
                }), 500
        else:
            return jsonify({
                "status": "failure",
                "message": f"Resume not found for candidate ID {candidate_id}"
            }), 404

    # Send the email with Base64 resumes
    error = send_selected_candidate_notification(
        selected_details=selected_details,
        recruiter_email=recruiter_email,
        subject=subject,
        body=body,
        base64_resumes=base64_resumes,
...
def send_selected_candidate_notification(selected_details, recruiter_email, subject, body, base64_resumes, sender_email, sender_password):
    # Retrieve mail server and port from the app configuration
    ...
    # Generate the email table rows
    table_rows = ""
    if selected_details:
        job_keys = selected_details[0].get('jobDetails', {}).keys()
        header_row = "<thead><tr><th>Serial No.</th>" + "".join([f"<th>{key.replace('_', ' ').title()}</th>" for key in job_keys]) + "</tr></thead>"
        table_rows += header_row
        for index, candidate in enumerate(selected_details, start=1):
            job_details = candidate.get('jobDetails', {})
            row = f"<tr><td>{index}</td>" + "".join([f"<td>{value if value else 'N/A'}</td>" for value in job_details.values()]) + "</tr>"
            table_rows += row

    # Define the updated HTML email body
    html_body = f"""
    <html>
    <head>
        <style>
    ...
    # Attach resumes as PDF
    for resume in base64_resumes:
        try:
            resume_data = base64.b64decode(resume["data"])
            part = MIMEApplication(resume_data, Name=resume["name"])
            part['Content-Disposition'] = f'attachment; filename="{resume["name"]}"'
            msg.attach(part)
        except Exception as e:
            return f"Failed to attach resume {resume['name']}: {str(e)}"
